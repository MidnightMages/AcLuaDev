...
public class ManagedMassStorageUD extends BaseMassStorageUD {
    final Config cfg;
    EphemeralFilesystemProxy fsProxy;
     ...
     */
    public ManagedMassStorageUD(String storageFamilyName, int totalCapacityBytes,
                                int diskId, Path rootPath, Config cfg) {
        super(storageFamilyName, "managed");
        this.totalCapacityBytes = totalCapacityBytes;
        this.cfg = cfg;
        fsProxy = new EphemeralFilesystemProxy(rootPath, !cfg.allowPhysicalFilesystemWrites());
        setBasePathAndConvertFs(diskId, rootPath);
    }

    // ############################### SETUP ###############################

    private void setBasePathAndConvertFs(int diskId, Path rootPath) {
        diskStorageId = diskId;
        this._dont_use_directly_fsRealBasePath = rootPath;
        RuntimeAssert.RuntimeAssert(fsProxy.isDirectory(rootPath), "the directory should already exist at this point");
        // testharness doesnt need to do this probably
//        try {
...
//        }
    ...
    public LuaFsFileUD open(String filePath, String mode) {
        ...
        if (fsProxy.isDirectory(getRealFsPath(filePath)))
        ...
        try (var objects = fsProxy.list(realFsPath)) {
            return LuaObject.tableFromArray(objects.map(x -> {
                var luaPath = decodeFilename(x.subpath(getFsRealBasePath().getNameCount() + depth, x.getNameCount()).toString())
                        .replace("\\", "/");
                return LuaObject.of(fsProxy.isDirectory(x) ? luaPath + "/" : luaPath);
    ...
    public boolean fileExists(String path) {
        ...
        return fsProxy.isRegularFile(realPath);
    }
    ...
    public boolean directoryExists(String path) {
        ...
        var realPath = getRealFsPath(normalizeEncodeAbsFilePath(path));
        return fsProxy.isDirectory(realPath);
    }
    ...
    public void delete(String path) {
        ...
        if (fsProxy.isRegularFile(realPath)) {
            ...
            try {
                fsProxy.delete(realPath);
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        } else if (fsProxy.isDirectory(realPath)) {
            var rootPath = getFsRealBasePath();
            try (Stream<Path> paths = fsProxy.walk(realPath, Integer.MAX_VALUE)) {
    ...
    public void makeDirectory(String path) {
        ...
        for (String segment : normalizedPath.split("/")) {
            ...
            if (fsProxy.isRegularFile(currentFilePath))
        ...
        try {
            fsProxy.createDirectories(getRealFsPath(normalizedPath));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public long spaceUsed(String path) {
        ...
        try (var paths = fsProxy.walk(realPath, Integer.MAX_VALUE)) {
            long[] totalByteCount = new long[]{0};
            paths.forEach(pi -> {
                if (fsProxy.isRegularFile(pi)) {
    ...
    private long getFileCost(Path realPath) {
        try {
            ...
            return openHandle != null ? openHandle.getUnflushedSize() : fsProxy.size(realPath);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private PathSearchResult getRealFileOrDirectoryPath(String virtualFsPath, boolean assertDirectory) {
        ...
        if (fsProxy.isDirectory(realPath))
            return new PathSearchResult(true, realPath);
        else if (fsProxy.isRegularFile(realPath)) {
            if (assertDirectory)
    ...
    public static String encodeFilename(String filename) {
        // not implemented on testharness yet
